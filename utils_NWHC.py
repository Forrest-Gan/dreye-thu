...
def getCoarse2FineModel_NWHC(summary=True):
    ...
    coarse_saliency_model.add(Conv3D(64, [3, 3, 3], activation='relu', padding='same', name='conv1', strides=(1, 1, 1)))
    coarse_saliency_model.add(MaxPooling3D(pool_size=(1, 2, 2), strides=(1, 2, 2), padding='valid', name='pool1'))
    coarse_saliency_model.add(Conv3D(128, [3, 3, 3], activation='relu', padding='same', name='conv2', strides=(1, 1, 1)))
    coarse_saliency_model.add(MaxPooling3D(pool_size=(2, 2, 2), strides=(2, 2, 2), padding='valid', name='pool2'))
    coarse_saliency_model.add(Conv3D(256, [3, 3, 3], activation='relu', padding='same', name='conv3a', strides=(1, 1, 1)))
    coarse_saliency_model.add(Conv3D(256, [3, 3, 3], activation='relu', padding='same', name='conv3b', strides=(1, 1, 1)))
    coarse_saliency_model.add(MaxPooling3D(pool_size=(2, 2, 2), strides=(2, 2, 2), padding='valid', name='pool3'))
    coarse_saliency_model.add(Conv3D(512, [3, 3, 3], activation='relu', padding='same', name='conv4a', strides=(1, 1, 1)))
    coarse_saliency_model.add(Conv3D(512, [3, 3, 3], activation='relu', padding='same', name='conv4b', strides=(1, 1, 1)))
    ...
    coarse_saliency_model.add(Conv2D(256, [3, 3], kernel_initializer='glorot_uniform', padding='same'))
    coarse_saliency_model.add(LeakyReLU(alpha=.001))
    coarse_saliency_model.add(UpSampling2D(size=(2, 2), data_format='channels_last'))
    coarse_saliency_model.add(BatchNormalization())
    coarse_saliency_model.add(Conv2D(128, [3, 3], kernel_initializer='glorot_uniform', padding='same'))
    coarse_saliency_model.add(LeakyReLU(alpha=.001))
    coarse_saliency_model.add(UpSampling2D(size=(2, 2)))
    coarse_saliency_model.add(BatchNormalization())
    coarse_saliency_model.add(Conv2D(64, [3, 3], kernel_initializer='glorot_uniform', padding='same'))
    coarse_saliency_model.add(LeakyReLU(alpha=.001))
    coarse_saliency_model.add(UpSampling2D(size=(2, 2), data_format='channels_last'))
    coarse_saliency_model.add(BatchNormalization())
    coarse_saliency_model.add(Conv2D(32, [3, 3], kernel_initializer='glorot_uniform', padding='same'))
    coarse_saliency_model.add(LeakyReLU(alpha=.001))
    coarse_saliency_model.add(UpSampling2D(size=(2, 2), data_format='channels_last'))
    coarse_saliency_model.add(BatchNormalization())
    coarse_saliency_model.add(Conv2D(16, [3, 3], kernel_initializer='glorot_uniform', padding='same'))
    coarse_saliency_model.add(LeakyReLU(alpha=.001))
    coarse_saliency_model.add(BatchNormalization())
    coarse_saliency_model.add(Conv2D(1, [3, 3], kernel_initializer='glorot_uniform', padding='same'))
    coarse_saliency_model.add(LeakyReLU(alpha=.001))
    ...
    x = concatenate([x, last_frame_bigger],axis=3)  # merge the last RGB frame
    ...
    x = LeakyReLU(alpha=.001)(x)
    x = Conv2D(4, [3, 3], padding='same', kernel_initializer='he_normal')(x)
    x = LeakyReLU(alpha=.001)(x)

    fine_saliency_model = Conv2D(1, [3, 3], padding='same', activation='relu')(x)
...
if __name__ == '__main__':
    ...
    model = getCoarse2FineModel_NWHC(summary=True)
